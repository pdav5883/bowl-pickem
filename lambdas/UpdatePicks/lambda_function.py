...
def lambda_handler(event, context):
    ...
    if data["lock_picks"]:
        return {"statusCode": 403, "body": "Picks are locked for this game"}
    ...
    try:
        ...
        assert (
            new_picks["name"] not in names
        ), f"{new_picks['name']} has already submitted picks"
        ...
        assert (
            len(new_picks["picks"]) == numgames
        ), "Incorrect number of picks submitted"

        # incorrect number of categories
        if "categories" in new_picks:
            assert (
                len(new_picks["categories"]) == numgames
            ), "Incorrect number of categories submitted"
        ...
        if "categories" in new_picks:
            ...
            assert all(
                c == 0 for c in catRemaining
            ), "Incorrect mix of categories submitted"
        ...
        if "categories" in new_picks:
            for cat in new_picks["categories"]:
                assert cat in (
                    ...
                    "5",
                    "6",
                ), "Category has incorrect format"

    except AssertionError as msg:
        print(msg)
        return {"statusCode": 400, "body": str(msg)}
    ...
    if "categories" in new_picks:
        data["players"].append(
            {
                "name": new_picks["name"],
                "picks": new_picks["picks"],
                "categories": new_picks["categories"],
            }
        )
    else:
