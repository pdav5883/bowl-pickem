...
function createBowlRow(bowl, bowlIndex, game, firstPlayoff) {
  ...
  game.players.forEach(player => {
    const pickCell = createPickCell(bowl, bowlIndex, player, game, firstPlayoff)
...
function createBowlInfoCell(bowl, bowlIndex, game, firstPlayoff) {
  ...
  // Teams
  const { team0Span, team1Span } = createTeamSpans(bowl, bowlIndex, game, firstPlayoff)
...
function stylePickCell(cell, bowl, bowlIndex, player, isPlayoff, game, firstPlayoff) {
  if (!isPlayoff) {
...
function stylePlayoffPickCell(cell, bowl, bowlIndex, player, game, firstPlayoff) {
  const pickIndex = player.picks[bowlIndex]
...
function calcScores(game) {
  ...
  const getPoints = (bowlIndex, playerIndex) => {
    ...
    return game.players[playerIndex].categories[bowlIndex] + game.bowls[bowlIndex].bonus
  }
...
function addBestFinishPopup() {
  ...
  tooltip.setAttribute("class", "d-none position-absolute bg-dark text-white text-center rounded p-2")
  tooltip.setAttribute("id", "bestmsg")
  tooltip.style.width = "260px"
  tooltip.style.top = "115%"
  tooltip.style.left = "100%"
  tooltip.style.zIndex = "1050"
  tooltip.textContent = "Shows each player's best possible final rank and how much they would win by (+) or lose by (-)"
